...
function changeXRecursive(data, jenis) {
  ...
  let huruf_pertama = +data[0];

  if (jenis === "ganjil" && (huruf_pertama % 2 === 0)) huruf_pertama = `x`;
  else if (jenis === "genap" && (huruf_pertama % 2 === 1)) huruf_pertama = `x`;
  
  data = data.slice(1);
  return huruf_pertama + changeXRecursive(data, jenis);
}
