...
function palindromeRecursive(sentence) {
    ...
    function cekPalindrome (kalimat = "", index = (kalimat.length - 1)) {
        if (index < 0) return "";
        return kalimat[index] + cekPalindrome(kalimat, index - 1);
    }

    let balik_sentence = cekPalindrome(sentence);
    
    return (sentence === balik_sentence) ? true : false;
}
