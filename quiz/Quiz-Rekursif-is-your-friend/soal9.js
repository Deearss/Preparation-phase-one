...
function virusCheckRecursive(str, viruses) {
  ...
  const arr_viruses = (viruses.length > 1) ? viruses.split("|") : [viruses[0]];
  const arr_str = str.split("");

  // hapus digit depan str dan sekalian tentukan apakah str[0] adalah virus yang terdaftar
  for (let i = 0; i < arr_viruses.length; i++) {
    if (arr_str[0].toLowerCase() === arr_viruses[i].toLowerCase()) {ketemu = true; break;}
  }
  str = arr_str.slice(1).join("");

  // jika setelah perulangan ditemukan virus maka result = 1
  if (ketemu === true) result = 1;

  return result + virusCheckRecursive(str, viruses);
}
